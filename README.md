...
Your content ideas, performance metrics, and preferences live in a local database (PostgreSQL or SQLite). You own the data, can export it, and can run ShipLog entirely offline for core workflows. No account or cloud sync required for the essentials.

### AI-Driven Iteration

ShipLog will connect to an LLM API to analyze your past video performance and stated content preferences, then suggest what to double down on, what to retire, and how to refine your next posts. Feedback is grounded in _your_ history, not generic creator advice.

### Trend Scraping

A niche-specific trend aggregator will surface high-performing topics and formats in your space, so you can spot opportunities early and align ideas with what's working without manually scrolling every platform and keeping track of trends yourself.
...
List a couple of ideas daily that users can upload based on their niche.
...
```bash
git clone git@github.com:ahmedsayidrashid/ShipLog
cd ShipLog
python -m venv .venv
